...
static void recalibrateBaselines()
{
    ...
    for (uint8_t i = 0; i < NUM_SENSORS; i++)
    {
        ...
        seedSensorState(sensorState[i], (float)filt, PROX_SEED_FRAMES, PROX_WARM_FRAMES); // filtered = new software baseline
...
void setup()
{
    ...
    proxCfg.baseWarmAlpha     = PROX_BASE_WARM_ALPHA;
...
void loop()
{
    ...
    // register's 4-count quantization caused ghost flutter / "pads popping out
    // of calibration", and its falling filter ate slow approaches). The
    // touch-status register is skipped too: nothing uses it (contact detection
    // is the jump detector). Net frame cost is now one burst per board,
    // ~2.6 ms of the 6 ms frame at 100 kHz.
    struct BoardData {
        uint8_t  filt[24];
    };
    ...
    for (uint8_t i = 0; i < NUM_SENSORS; i++)
    {
        ...
        // PAD_DEADBAND_TRIM raises one noisy pad's gate without touching the rest.
        float intensity;
        bool  isTouch;
