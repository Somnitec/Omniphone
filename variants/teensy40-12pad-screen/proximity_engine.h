...
inline void updateProximity(float rawDelta, uint32_t nowMs,
    ...
    // (tiny steps) keeps the gentle smoothing. Falling stays slow (sustain).
    float riseStep = rawDelta - state.fast; // this frame's change in the raw signal
    float alpha;
    if (riseStep > 0.0f) {
    ...
    float sa = (target > state.intensitySm) ? cfg.smoothAlphaRise : cfg.smoothAlpha;
