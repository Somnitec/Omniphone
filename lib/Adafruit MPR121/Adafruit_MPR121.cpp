...
bool Adafruit_MPR121::begin(uint8_t i2caddr, TwoWire *theWire,
  ...
  setThresholds(touchThreshold, releaseThreshold, sensorAmount);
  ...
  // ELE_EN Electrode Enable:  amount of electrodes running (sensorAmount)
  byte ECR_SETTING = 0b10000000 + sensorAmount;
 ...
 * release)
 */
void Adafruit_MPR121::setThreshholds(uint8_t touch, uint8_t release, uint8_t sensorAmount) {
  setThresholds(touch, release, sensorAmount);
}
 ...
 */
void Adafruit_MPR121::setThresholds(uint8_t touch, uint8_t release, uint8_t sensorAmount) {
